...
PYTHON_VARS_AFTER_PYTEST ?= -m '$(MARK)' --json-report --json-report-file=build/report.json --junitxml=build/report.xml --event-storage-files-path="build/events" --pointing-files-path=build/pointing --event-diag-file-dir=build/ --track-device-events "mid-dish/dish-manager/SKA001" --track-device-events "mid-dish/ds-manager/SKA001"
