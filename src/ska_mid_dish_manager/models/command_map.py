...
def update_task_status(task_callback: Optional[Callable], **task_statuses) -> None:
    """Wraps the task callback to report progress."""
    if task_callback:
...
class CommandMap:
    ...
    def _run_long_running_command(
        ...
        for device, fan_out_args in commands_for_sub_devices.items():
            ...
            reply = self._fan_out_cmd(task_callback, device, fan_out_args)
            if reply[0] == TaskStatus.FAILED:
                failed_cmds[device] = f"{device}.{cmd_name}"
                error_messages.append(reply[1])
            else:
                successful_cmds[device] = f"{device}.{cmd_name}"

        if failed_cmds:
            cmds = ", ".join(failed_cmds.values())
            error_messages = "; ".join(error_messages)
            msg = f"DishManager.{running_command} failed: {cmds} failed to execute"
            self.logger.info(f"{msg}. Errors: {error_messages}")
            update_task_status(
                task_callback,
                progress=msg,
                status=TaskStatus.FAILED,
