...
def dish_freq_band_configuration(
    ...
    class _BandSelector:
        def go_to_band(self, band_number):
            ...
            try:
                # wait for events
                event_store.wait_for_value(Band(int(band_number)), timeout=15)
                LOGGER.info(
...
def modes_helper(
    ...
    class _ModesHelper:
        ...
        def dish_manager_go_to_mode(self, mode_name):
            ...
            if (
                mode_name == "OPERATE"
                and dish_manager.configuredBand.name in ["NONE", "UNKNOWN"]
            ):
            ...
            try:
                # wait for events
                event_store.wait_for_value(DishMode[mode_name], timeout=15)
                LOGGER.info(
