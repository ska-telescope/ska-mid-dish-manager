...
@scenario(
    ...
    "LMC Report DSH Capability Standby when dishMode is STANDBY-FP",
)
...
def configure_band(
    ...
    # pylint: disable=missing-function-docstring
    # FIXME: Only band2 can be configured from DishManager. Will need
    # to skip subsequent tests which check that CONFIG is reported briefly.
    # Since dish will already be in band 2 and ONFIG change event will never
    # come through. Alternatively the helper method can switch to a different
    # band and back to requested band. For now add this as xfail marker.
    # Same for all subsequent tests which check that CONFIG is reported briefly
