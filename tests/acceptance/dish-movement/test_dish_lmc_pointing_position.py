...
def dish_reports_allowed_pointing_state(
...
@when(
    parse(
        "I issue Track on dish_manager to move azimuth "
        "and elevation by {delta:g} degrees each"
    )
)
def request_pointing_position(delta, dish_manager):
    ...
    while dish_far_from_requested_position:
        ...
        dish_close_to_configured_threshold = (
...
@then(
    parse("dish_manager pointingState should transition to {track} on target")
)
def check_pointing_state_on_target(
...
@then(
    ...
    " the same achieved pointing position"
)
def check_dish_manager_dish_structure_position(
    ...
    assert list(dish_manager_achieved_az_el) == list(
        dish_structure_achieved_az_el
    )

    LOGGER.info(
        f"{dish_manager} and {dish_structure} devices report the same achieved position"
    )
