...
def test_healthinfo(dish_manager_proxy, spfrx_device_proxy, event_store_class):
    ...
    event_store.wait_for_value(['mid-dish/simulator-spfrx/SKA001: ["Unknown failure reason"]'])
    spfrx_device_proxy.write_attribute("healthState", HealthState.DEGRADED)
    event_store.wait_for_value(['mid-dish/simulator-spfrx/SKA001: ["Unknown degraded reason"]'])
