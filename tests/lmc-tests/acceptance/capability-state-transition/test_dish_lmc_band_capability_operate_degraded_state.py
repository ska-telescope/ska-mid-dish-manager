...
def check_spf_band_capability_transient_state(band_number, expected_state, spf, spf_event_store):
    # pylint: disable=missing-function-docstring
    ...
    spf_events = spf_event_store.get_queue_values(timeout=10)
...
def check_dish_manager_band_capability_transient_state(
    ...
    assert dm_cap_state_events, str(datetime.datetime.fromtimestamp(time.time()))
