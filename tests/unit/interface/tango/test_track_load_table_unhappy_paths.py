...
def test_track_load_table_unhappy_paths(
    ...
    # the behavior of `get_current_tai_offset_with_manual_fallback`
    ...
    # `get_current_tai_offset_with_manual_fallback` raises a DevFailed error
    with patch.object(
    ...
    # Test that programTrackTable still works if the `execute_command` of
    # `get_current_tai_offset_with_manual_fallback` raises a ConnectionError
    with patch.object(
        dish_manager_cm.sub_component_managers["DS"], "execute_command"
    ) as mock_execute_command:
