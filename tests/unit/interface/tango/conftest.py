    ...
    with (
        ...
        for method_name in candidate_stub_methods:
            if method_name == "execute_command":
                ...
                mock_method = Mock(return_value=(json.dumps({"uid": "12345", "result": "some result message", "status": TaskStatus.COMPLETED.name}),))
            else:
                mock_method = Mock()
...
@pytest.fixture
def dish_manager_resources_with_b5dc_monitoring():
    with (
        patch("ska_mid_dish_manager.component_managers.spfrx_cm.MonitorPing"),
        patch(
            "ska_mid_dish_manager.component_managers.tango_device_cm."
