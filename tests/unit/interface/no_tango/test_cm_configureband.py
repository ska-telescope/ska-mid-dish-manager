...
def test_configureband_json_handler_band_2(
    component_manager: DishManagerComponentManager,
    mock_command_tracker: Mock,
    callbacks: dict,
) -> None:
    """Verify behaviour of ConfigureBand for Band 2.
    ...
    band_index = "2"
    data = {
        "receiver_band": band_index,
        "spfrx_processing_parameters": {"dishes": ["all"], "sync_pps": True},
    }
    arg = json.dumps(data, indent=4)
    with patch("json.dumps", Mock(return_value="mocked sub-device-command-ids")):
        component_manager.configure_band_with_json(arg, callbacks["task_cb"])
        # wait a bit for the lrc updates to come through
        component_state_cb.get_queue_values()
    ...
    expected_call_kwargs = (
        ...
        {"progress": f"SetIndexPosition called on DS, ID {mock_command_tracker.new_command()}"},
        {"progress": "Awaiting DS indexerposition change to B2"},
        {"progress": f"ConfigureBand called on SPFRX, ID {mock_command_tracker.new_command()}"},
        {"progress": "Awaiting SPFRX configuredband change to B2"},
        {"progress": "Commands: mocked sub-device-command-ids"},
        {"progress": "Awaiting configuredband change to B2"},
    )
    ...
    task_cb = callbacks["task_cb"]
    task_cb.assert_called_with(
        progress="ConfigureBand completed",
        status=TaskStatus.COMPLETED,
        result=(ResultCode.OK, "ConfigureBand completed"),
    )
