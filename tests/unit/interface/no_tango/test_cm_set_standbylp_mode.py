...
def test_set_standbylp_handler(
    ...
    expected_call_kwargs = (
        ...
        {"progress": "Awaiting SPF operatingmode change to [<SPFOperatingMode.STANDBY_LP: 2>]"},
        {"progress": f"SetStandbyMode called on SPFRX, ID {mock_command_tracker.new_command()}"},
        {"progress": "Awaiting SPFRX operatingmode change to [<SPFRxOperatingMode.STANDBY: 2>]"},
        {"progress": f"SetStandbyLPMode called on DS, ID {mock_command_tracker.new_command()}"},
        {"progress": "Awaiting DS operatingmode change to [<DSOperatingMode.STANDBY_LP: 2>]"},
